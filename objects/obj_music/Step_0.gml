	...
	&& !instance_exists(obj_levelsettings) && !instance_exists(obj_titlecard) && !instance_exists(obj_rank) && room != timesuproom)
	{
		...
		else if fmod_event_instance_is_playing(panicmusicID)
		{
			...
			else if MOD.CosmicClones
			{
				...
				else if check_lap_mode(LAP_MODES.laphell) && global.laps >= 3 // Absolute AbsurZiti
					fmod_event_instance_set_parameter(panicmusicID, "state", 4, true);
			...
			{ 
				if check_lap_mode(LAP_MODES.laphell) && global.laps >= 2 // lap 3 songs
					fmod_event_instance_set_parameter(panicmusicID, "state", 3, true);
				else if check_lap_mode(LAP_MODES.laphell) && global.laps >= 3 // lap 4 songs
					fmod_event_instance_set_parameter(panicmusicID, "state", 4, true);
				else if check_lap_mode(LAP_MODES.infinite) && global.laps >= 4 // Death Wish
					fmod_event_instance_set_parameter(panicmusicID, "state", 6, true);
